...
export interface IGamestate {
  ...
  hp1: number;
  hp2: number;
  // textExplanationOfLastRound?: string;
  logs: IFightLogLine[];
}
...
export const Fightengine = {
  init: (char1: Character, char2: Character): IGamestate => ({
    char1: char1,
    char2: char2,
    currStep: 0,
  ...
  getNextStep: (state: IGamestate): IGamestate => {
    ...
    return {
      ...
      // textExplanationOfLastRound: `${state.stepsList[state.currStep]}s ${
      //   timeToHit(state.stepsList[state.currStep], state.char2.delay)
      //     ? `${state.char2.name.split(",")[0]} lyö!`
      //     : `${state.char1.name.split(",")[0]} lyö!`
      // }`
    };
  }
};
